...
public class SurveysController {
	...
	public String index(Model model, Authentication auth, Integer surveyId) {
		...
		List<Answers> answers = answerService.selectAnswersByUserIdSurveyId(param);
		
		Integer qTotalVoteCount = 0;
		for (Questions questionItem : questions) {
			List<Options> qOptions = questionItem.getOptions();
			
			for(Answers answerItem : answers) {
				if(questionItem.getId().equals(answerItem.getQuestionid())) {
					if(questionItem.getType() == 0) {
						// 객관식
						for(Options optionItem : qOptions) {
							if(optionItem.getOption().equals(answerItem.getAnswer())) {
						...
						// 주관식
						questionItem.setAnswerValue(answerItem.getAnswer());
					}
				}
			}
			...
			for(Options optionItem : qOptions) {
				...
				countParam.put("surveyid", surveyId);
				countParam.put("questionid", optionItem.getQuestionid());
				countParam.put("answer", optionItem.getOption());
				
				Integer voteCount = answerService.countAnswerBySurveyIdQustionidAnswer(countParam);
				
				optionItem.setVoteCount(voteCount);
				
				qTotalVoteCount += voteCount;
			}
			
			questionItem.setTotalVoteCount(qTotalVoteCount);
			qTotalVoteCount = 0;
		}
	...
	public void submitAnswer(/*RequestArrayVO req, List<Answers> answer, */String reqJson, Authentication auth) {
		...
		if(answer != null) {
			...
			Integer firstParentPoint = (int) Math.floor((totalAmount/10)*ProjectConstants.FIRST_PARENT_POINT_RATE);
			Integer secondParentPoint = (int) Math.floor((totalAmount/10)*ProjectConstants.SECOND_PARENT_POINT_RATE);
			...
			firstParent.setPoint(firstParent.getPoint()+firstParentPoint);
			userService.updateUser(firstParent);
			
			trading.setAmount(firstParentPoint);
			...
			secondParent.setPoint(secondParent.getPoint()+secondParentPoint);
			userService.updateUser(secondParent);
			
			trading.setAmount(secondParentPoint);
